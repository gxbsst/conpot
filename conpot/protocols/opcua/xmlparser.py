...
class XMLParser(object):
    ...
    def __init__(self, ua_server, xmlpath):
    ...
    def __next__(self):
        while True:
            ...
            # else:
            node = self._parse_node(child.tag, child)
            return node
    ...
    def _update_node_value(self, node_id):
        node = self.ua_server.get_node(node_id)
        if node:
            node.set_value(conpot_core.get_databus().get_value(node_id))

    def _find_data_bus_value(self, matched):
        databus_key = matched.groups()[1][2:-1]
        conpot_core.get_databus().observe_value(databus_key, self._update_node_value)
        return str(conpot_core.get_databus().get_value(databus_key))
    ...
    def _parse_value(self, el, obj):
        for val in el:
            ...
            # 取databus中的值进行替换
            val.text = re.sub(r"(.*)(\$\{.*\})(.*)", self._find_data_bus_value, val.text)
